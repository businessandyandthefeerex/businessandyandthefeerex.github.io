...
{% assign posts = site.posts | where: "country", "New Zealand" | where: "region", "Bay of Plenty" | where: "city", "Mount Maunganui" %}
...
{% for group in sorted_suburb_groups %}
  {% if group.name != "" %}
    <h4><a href="/country/new-zealand/bay-of-plenty/{{ group.name | downcase | slugify }}" style="color: var(--heading-color);">{{ group.name }}</a></h4>
  {% else %}
