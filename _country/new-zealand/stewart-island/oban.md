...
{% assign posts = site.posts | where: "country", "New Zealand" | where: "region", "Stewart Island" | where: "city", "Oban" %}
...
{% for group in sorted_suburb_groups %}
  {% if group.name != "" %}
    <h4><a href="/country/new-zealand/stewart-island/{{ group.name | downcase | slugify }}/" style="color: var(--heading-color);">{{ group.name }}</a></h4>
  {% else %}
