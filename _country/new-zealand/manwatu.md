---
layout: page
title: Manwatu, New Zealand
country: New Zealand
region: Manwatu
permalink: /country/new-zealand/manwatu/
---
[↑ Go to New Zealand regions](/country/new-zealand/)

{% assign posts = site.posts | where: "region", "Manwatu" | where: "country", "New Zealand" %}
...
{% for city_group in sorted_city_groups %}
  ...
  {% if city_group.name != "" %}
    <h3><a href="/country/new-zealand/manwatu/{{ city_slug }}" style="color: var(--heading-color);">{{ city_group.name }}</a></h3>
  {% else %}
  ...
  {% for group in sorted_suburb_groups %}
    ...
    {% if group.name != "" %}
      <h4><a href="/country/new-zealand/manwatu/{{ suburb_slug }}" style="color: var(--heading-color);">{{ group.name }}</a></h4>
    {% else %}
