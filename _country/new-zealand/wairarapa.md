...
{% for city_group in sorted_city_groups %}
  ...
  {% if city_group.name != "" %}
    <h3><a href="/country/new-zealand/wairarapa/{{ city_slug }}/" style="color: var(--heading-color);">{{ city_group.name }}</a></h3>
  {% else %}
  ...
  {% for group in sorted_suburb_groups %}
    ...
    {% if group.name != "" %}
      <h4><a href="/country/new-zealand/wairarapa/{{ suburb_slug }}/" style="color: var(--heading-color);">{{ group.name }}</a></h4>
    {% else %}
